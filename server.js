...
io.on("connection", (socket) => {
  socket.on("joinChannel", ({ username, channel }) => {
    ...
    socket.emit(
      "message",
      messageFormat(`--Chatgram Bot--`, "Welcome to Chatgram!")
    );
        ...
        for (let i = 0; i < predictions.length; i++) {
          if (predictions[i].results[0].match === true) {
            ...
            return socket.emit(
              ...
              messageFormat(
                `--Chatgram Bot--`,
                "Your message has been deleted as it violates our anti-toxicity guidelines. Please try to be respectful to others !"
              )
            );
          }
        }
